...
###################################################################################################################################################################    
    ...
    def Infections_t(self,G_next,p_i,acc_inf):
    ...
    def Infections_Box(self,G_next,p_i,acc_inf):
          
          for infected_node in self.list_of_state(G_next,'I'):
            susceptible_neighbors = self.intersection(self.list_of_state(G_next,'S'), list(G_next.neighbors(infected_node)) )
            #print(susceptible_neighbors)
            if len(susceptible_neighbors) != 0:
                num_new_inf_neighbors = np.random.binomial(len(susceptible_neighbors),p_i)
                #print(num_new_inf_neighbors)
                for iteracion in range(num_new_inf_neighbors):
                    caja = []
                    for nodo_sus in susceptible_neighbors:
                        wt = G_next[infected_node][nodo_sus]["weight"]
                        caja = caja + [nodo_sus]*wt
                    #print(caja)
                    new_infected = random.choice(caja)
                    self.change_state([new_infected],G_next,'I')
                    susceptible_neighbors.remove(new_infected)      
    ...
    def Recovered_to_Susceptible(self,G_next,recovering_period,p_s):
        Possible_New_Sus = self.Count_Days_I_R(G_next, recovering_period, 'R')
        
        if Possible_New_Sus !=0:
            #num_new_sus = np.random.binomial(len(Possible_New_Sus),p_s)
            #New_Sus = random.sample(Possible_New_Sus,num_new_sus)
    ...
    def Update_Epidemic(self,G_array,p_i,acc_inf,infectious_period,p_r,recovering_period,p_s):
        '''
        This Function updates the state of the nodes of the graph with a stochastical process
        It selects the nodes with an infected state ('state=I') and then it makes a list of the susceptible neighbors
        of that infected node. With a binomial distribution, it then selects how many nodes will be infected by an
        ...
        acc_inf = self.Infections_Box(G_next,p_i,acc_inf)
        ...
        self.Recovered_to_Susceptible(G_next,recovering_period,p_s)
    ...
    def Epidemic_Evolution(self,G0,p_i,r_period,p_r,inmmunity_period,p_s):
        ...
        Accumulative_count = [acc_inf]      # initialize the general array of accumulated infectections over time
        for step_i in tqdm(range(1,self.steps)):
            Gtrack, acc_inf = self.Update_Epidemic(Gtrack,p_i,acc_inf,r_period,p_r,inmmunity_period,p_s)   #Update the Graphs and number of accumulated infections in every period of time
            Accumulative_count.append(acc_inf)      # Add the number of accumulated infections of every period to the general array 
