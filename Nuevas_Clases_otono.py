...
###################################################################################################################################################################    
    ...
    def Initial_Graph3(self,N_infected,graph_name):
        '''
        ...
        self.add_state_days(seeds, G, 'days_I')
    
        return G
    
    def add_state_days(self,Nodes_of_Certain_State,G,I_R_days):
        '''
        This Function changes the state of the nodes of a certain group of nodes introduced in the input 
            Input:
                    Nodes_of_Certain_State - List of nodes that will change state
    ...
    def Count_Days_I_R(self,G,r_period,state_counted):
        List_Nodes_Days_I_R = []
        atribute = 'days_'+state_counted
        for person in self.list_of_state(G, state_counted):
            if G.nodes[person][atribute] >= r_period:
                List_Nodes_Days_I_R.append(person)
        return List_Nodes_Days_I_R
    
        
    def Update_State3(self,G_array,p_i,acc_inf,recoverable_period,p_r):
        '''
        ...
        self.add_state_days(self.list_of_state(G_next,'I'), G_next, 'days_I')
        self.add_state_days(self.list_of_state(G_next,'R'), G_next, 'days_R')
        
        return G_array,acc_inf
    
    
    def Epi_Evolution3(self,G0,p_i,r_period,p_r):
        ...
        for step_i in tqdm(range(1,self.steps)):
            Gtrack, acc_inf = self.Update_State3(Gtrack,p_i,acc_inf,r_period,p_r)   #Update the Graphs and number of accumulated infections in every period of time
